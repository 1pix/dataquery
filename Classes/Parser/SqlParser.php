...
use Tesseract\Dataquery\Utility\QueryObject;
...
class SqlParser
{
    ...
    /**
     * @var QueryObject Structured type containing the parts of the parsed query
     */
     ...
     * @return QueryObject An object containing the parsed query information
     */
    public function parseSQL($query)
    {
        $this->queryObject = GeneralUtility::makeInstance(
                QueryObject::class
        );
            ...
            switch ($keyword) {
                ...
                case 'RIGHT JOIN':
                    ...
                    if (!isset($this->queryObject->structure['JOIN'])) {
                        $this->queryObject->structure['JOIN'] = array();
                    }
                ...
                case 'LIMIT':
                    if (strpos($value, ',') !== false) {
                        ...
                        $this->queryObject->structure['OFFSET'] = (int)$limitParts[0];
                        $this->queryObject->structure[$keyword] = (int)$limitParts[1];
                    } else {
                        $this->queryObject->structure[$keyword] = (int)$value;
                    }
                    break;
                case 'OFFSET':
                    $this->queryObject->structure[$keyword] = (int)$value;
    ...
    public function parseSelectStatement($select)
    {
        ...
        for ($i = 0; $i < $stringLenth; $i++) {
            ...
            switch ($character) {
                ...
                case ',':
                    ...
                    if ($openBrackets == 0) {
                        $this->parseSelectField(trim($currentField), $lastBracketPosition, $hasFunctionCall,
                                $hasWildcard);
                        $currentField = '';
                        $hasFunctionCall = false;
                        $hasWildcard = false;
        ...
        if ($openBrackets > 0) {
            throw new InvalidQueryException('Bad SQL syntax, opening and closing brackets are not balanced',
                    1272954424);
        } else {
     ...
     * @return void
     */
    protected function parseSelectField(
            $fieldString,
            $lastBracketPosition = 0,
            $hasFunctionCall = false,
            $hasWildcard = false
